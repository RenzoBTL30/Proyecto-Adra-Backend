...
public interface PreguntaRepository extends JpaRepository<Pregunta, Integer>{
		...
		// Listar
		@Query(value = "{call SP_LIST_TBL_PREGUNTA()}", nativeQuery = true) 
		List<Pregunta> listaPreguntas();
		
		
		// Buscar
		@Query(value = "{call SP_BUSCAR_TBL_PREGUNTA(:p_nIDPREG)}", nativeQuery = true) 
		Optional<Pregunta> buscarPregunta(@Param("p_nIDPREG") int p_nIDPREG);
		
		
		// Insertar
		@Query(value = "{call SP_INS_TBL_PREGUNTA(:p_vDE_PREGUNTA, :p_nID_RECURSO, @V_ID)}", nativeQuery = true) 
		String insertarPregunta(@Param("p_vDE_PREGUNTA") String p_vDE_PREGUNTA,
